...
export default function MateriPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const allData = await getFullCourses();
        const currentCourse = allData.find(c => c.id === Number(params.id));
        
        if (currentCourse) {
          setCourse(currentCourse);
          setModules(currentCourse.modules || []);
          const allMateri = currentCourse.modules.flatMap(m => m.materi);
          const currentIndex = allMateri.findIndex(m => m.id === Number(params.materiId));
  ...
  }, [params.id, params.materiId, router]);
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-slate-950 text-slate-200 flex flex-col overflow-hidden">
      <header className="h-16 bg-slate-900 border-b border-slate-800 px-8 flex items-center shrink-0">
        ...
        <h2 className="text-2xl font-black tracking-tight uppercase text-white">{course?.title}</h2>
      </header>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto bg-slate-950 flex flex-col relative">
          <div className="w-full px-12 py-12 flex-1">
            <h1 className="text-4xl font-black text-white mb-8 tracking-tighter uppercase">{materi?.title}</h1>
            ...
            <div className="text-slate-300 text-xl leading-relaxed mb-20 bg-slate-900/30 p-10 rounded-[48px] border border-slate-800 font-sans">
            ...
            {materi?.has_reflection && (
              <div className="mt-16 p-10 rounded-[40px] bg-purple-600/5 border border-purple-500/20 shadow-2xl relative">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <span className="text-xs font-black text-purple-400 uppercase tracking-widest">Refleksi</span>
                </div>
                ...
                <div className="mt-8 flex justify-end">
                  <button onClick={handleSendAssignment} className={`${isSubmitted ? "bg-slate-800 text-slate-400" : "bg-purple-600 text-white"} px-10 py-4 rounded-2xl font-black text-xs uppercase tracking-widest`}>
            ...
            {materi?.assignment && (
              <div className="mt-16 p-10 rounded-[40px] bg-blue-600/5 border border-blue-500/20 shadow-2xl flex flex-col md:flex-row items-center gap-10">
                ...
                <div className="flex-1">
                  <h4 className="text-white font-black text-3xl uppercase tracking-tighter mb-2">Tugas: {materi.assignment.type}</h4>
                  <p className="text-slate-400 text-lg mb-8 italic">"{materi.assignment.instruction}"</p>
                  <button onClick={() => setShowWorkspace(!showWorkspace)} className="px-10 py-4 bg-blue-600 rounded-2xl font-black text-xs uppercase tracking-widest text-white">
            ...
            {showWorkspace && materi?.assignment && (
              <div className="mt-10 bg-slate-900 border border-slate-800 rounded-[40px] overflow-hidden flex flex-col h-[800px] shadow-2xl">
                <div className="h-16 bg-slate-800/50 border-b border-slate-700 px-10 flex justify-between items-center">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{materi.assignment.type} Editor</span>
                ...
                <div className="flex-1 flex overflow-hidden">
                  {materi.assignment.type === 'flowchart' ? (
                    ...
                    <div className="flex-1 flex overflow-hidden font-mono">
                      <div className="flex-1 border-r border-slate-800"><Editor height="100%" defaultLanguage="c" theme="vs-dark" value={userCode} onChange={(v) => setUserCode(v)} options={{ fontSize: 16, minimap: { enabled: false }, readOnly: isSubmitted }} /></div>
                      <div className="w-96 bg-slate-950 p-8 text-sm text-slate-500 italic">
                        <div className="mb-4 text-blue-500 font-black uppercase tracking-widest">Run: {runCount}</div>
            ...
            <div className="mt-32 flex justify-between items-center border-t border-slate-900 pt-16">
              {prevMateriId ? <Link href={`/student/courses/${params.id}/materi/${prevMateriId}`} className="text-slate-500 hover:text-white font-black text-xs uppercase tracking-widest flex items-center gap-3"><ArrowLeft size={20} /> Back</Link> : <div/>}
              
              {nextMateriId ? (
                <button disabled={!canGoNext} onClick={() => router.push(`/student/courses/${params.id}/materi/${nextMateriId}`)} className={`font-black text-xs uppercase tracking-widest flex items-center gap-3 px-12 py-5 rounded-[24px] ${!canGoNext ? "bg-slate-900 text-slate-700 opacity-50" : "bg-blue-600 text-white"}`}>Next Materi <ArrowRight size={20} /></button>
              ) : (
                <button onClick={() => isPosttestDone ? router.push('/student/courses') : router.push(`/student/test/${params.id}/posttest/${posttestId}`)} className={`px-12 py-5 rounded-[24px] font-black uppercase tracking-widest flex items-center gap-3 ${isPosttestDone ? "bg-slate-800 text-white" : "bg-orange-600 text-white"}`}>
                  {isPosttestDone ? "Keluar Course" : "Post-test Final"} <ArrowRight size={20} />
                </button>
              )}
            </div>
          </div>
