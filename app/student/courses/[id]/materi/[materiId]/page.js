...
export default function MateriPage() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (currentCourse) {
          ...
          setTimeLeft(5);
          setUserReflection("");
          setSelectedObjectives([]); // 👈 Reset centang saat pindah materi
          setRunCount(0);
          setUserInput("");
          ...
          try {
            ...
            if (subData.exists) {
              ...
              // ✅ RESTORE: Ambil data centang dari database
  ...
  // ✅ LOGIKA BARU: Handle Klik Checkbox
  const handleObjectiveTick = (objectiveText) => {
    if (isSubmitted) return; // Tidak bisa diubah jika sudah submit
  ...
  const handleSendAssignment = async () => {
    ...
    try {
      // ✅ PAYLOAD: Kirim achieved_objectives ke backend
      const res = await api.post("/api/student/submit-assignment", { 
        materi_id: materi.id, 
        content: { 
            task: taskContent, 
            reflection: userReflection,
            achieved_objectives: selectedObjectives // 👈 Data centang
        },
        run_count: runCount
      });
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto bg-slate-950 flex flex-col relative custom-scrollbar">
          <div className="w-full px-12 py-12 flex-1">
            ...
            {/* ✅ BAGIAN REFLEKSI DENGAN CHECKBOX */}
            ...
            {/* Assignment & Workspace Area (UI Tetap Sama) */}
            {materi?.assignment && (
