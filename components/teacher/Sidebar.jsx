...
import {
  ...
  ChevronDown,
  Star,
  GraduationCap, // Ikon baru untuk menu Nilai
} from "lucide-react";
...
export default function TeacherSidebar({ isOpen, setIsOpen }) {
  ...
  // Otomatis buka dropdown berdasarkan pathname saat ini
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      <div className={`flex items-center mb-10 ${isOpen ? "justify-between" : "justify-center"}`}>
        {isOpen && (
          <h2 className="text-xl font-bold text-blue-500 animate-in fade-in duration-500">
            Teacher Panel
          </h2>
        )}
      ...
      <ul className="space-y-3">
        {menu.map((item) => {
          ...
          const isActive = pathname.startsWith(item.href) || (item.subItems?.some(sub => pathname.startsWith(sub.href)));
          ...
          if (item.isDropdown) {
            return (
              <li key={item.name} className="relative group">
                      ...
                      <li key={sub.href}>
                        <Link
                          ...
                          className={`block py-2 px-3 rounded-lg text-sm transition-colors ${
                            pathname.startsWith(sub.href) ? "text-blue-400 font-bold bg-blue-400/5" : "text-slate-500 hover:text-white"
                          }`}
                        >
                          {sub.name}
                        </Link>
                ...
                {!isOpen && (
                  <div className="absolute left-14 top-1/2 -translate-y-1/2 bg-slate-800 text-white text-xs px-3 py-2 rounded-md shadow-xl invisible opacity-0 group-hover:visible group-hover:opacity-100 transition-all z-[60] border border-slate-700 whitespace-nowrap">
                    {item.name}
                  </div>
                )}
              </li>
          ...
          return (
            <li key={item.href} className="relative group">
              ...
              {!isOpen && (
                <div className="absolute left-14 top-1/2 -translate-y-1/2 bg-slate-800 text-white text-xs px-3 py-2 rounded-md shadow-xl invisible opacity-0 group-hover:visible group-hover:opacity-100 transition-all z-[60] whitespace-nowrap border border-slate-700">
                  {item.name}
                </div>
              )}
            </li>
